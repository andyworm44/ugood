...
function HomeScreen({ navigation }: any) {
  ...
  console.log('🏠 HomeScreen 狀態:', { hasSharedToday, matchStatus });
    ...
    <SafeAreaView style={styles.homeContainer}>
      ...
      <View style={{ backgroundColor: 'red', padding: 20, margin: 10 }}>
        <Text style={{ color: 'white', fontSize: 18, textAlign: 'center' }}>
          🎯 HomeScreen 正在渲染！用戶: {currentUser?.email}
        </Text>
      </View>
...
function AppContent() {
  ...
  console.log('🔍 AppContent - currentUser:', currentUser?.email, 'isLoading:', isLoading);

  // 添加額外的安全檢查
  if (isLoading === undefined || isLoading === null) {
    console.log('⚠️ isLoading 狀態異常，強制設為 false');
    // 不返回載入畫面，直接繼續
  } else if (isLoading) {
    return (
      <View style={[styles.container, { justifyContent: 'center', alignItems: 'center' }]}>
        <Text style={{ fontSize: 18, color: '#4CAF50' }}>載入中...</Text>
      </View>
    );
  }

  console.log('🎯 準備渲染導航 - currentUser存在:', !!currentUser);
  console.log('🎯 初始路由將是:', currentUser ? "Home" : "Welcome");
